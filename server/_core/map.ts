...
function getMapsConfig(): MapsConfig {
  // Use direct Google Maps API key, fall back to Forge proxy
  const directKey = ENV.googleMapsApiKey;
  if (directKey) {
    return { baseUrl: "https://maps.googleapis.com", apiKey: directKey };
  }
...
export async function makeRequest<T = unknown>(
  ...
  // Construct full URL — direct Google API or Forge proxy
  const isDirectGoogle = baseUrl.includes("googleapis.com");
  const url = new URL(isDirectGoogle ? `${baseUrl}${endpoint}` : `${baseUrl}/v1/maps/proxy${endpoint}`);
